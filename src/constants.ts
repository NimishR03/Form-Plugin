import { checkLabel, convertToUpperSnakeCase, converttoCamelCase, obtainName } from "./util";

export function obtainConstants(component) {
    let code;
    if (checkLabel(component)) {
return `    ${convertToUpperSnakeCase(obtainName(component))}: '${converttoCamelCase(obtainName(component))}'`;
    } 
    else {
